...
if [ ! -r "$node_path_file" ]; then
  printf '%s\n' 'codexy notification hook skipped: missing Node.js path' >&2
  exit 0
fi

IFS= read -r node_path < "$node_path_file"
if [ ! -x "$node_path" ]; then
  printf '%s\n' 'codexy notification hook skipped: Node.js is unavailable' >&2
  exit 0
fi

exec "$node_path" "$runtime_directory/notify_mobile.mjs"
